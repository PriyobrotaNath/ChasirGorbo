...
if(isset($_POST['submit'])){
	...
	if(count($errors)==0){
		...
		$sql="INSERT INTO exporter(NAME,CONTACT,NID,E_MAIL,ADDRESS,POSTAL_CODE,LICENSE_NO,PASSWORD) 
				VALUES ('$NAME','$CONTACT','$NID','$E_MAIL','$ADDRESS',$POSTAL_CODE,$LICENSE_NO,'$PASS')";
		mysqli_query($db,$sql);
		$_SESSION['CONTACT']=$CONTACT;
		$_SESSION['success']="";
		header('location:exporter_home.php');
 ...
 if(isset($_POST['Sign In'])){
	...
	if(count($errors)==0){
		$PASSWORD=md5($PASS);
		$query="SELECT * FROM exporter WHERE CONTACT='$CONTACT' AND PASSWORD='$PASS'" ;
